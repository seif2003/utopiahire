...
import { Loader2, Plus, Eye, Users, ArrowLeft, Trash2, Edit, Building2 } from 'lucide-react'
...
interface OrganizationJobsContentProps {
  organization: Organization
  userId: string
}

export function OrganizationJobsContent({ organization, userId }: OrganizationJobsContentProps) {
  ...
  const fetchOrganizationJobs = async () => {
    ...
    try {
      const url = filter === 'all' 
        ? `/api/organizations/${organization.id}/jobs` 
        : `/api/organizations/${organization.id}/jobs?status=${filter}`
        ...
        <div className="flex items-center gap-4 mb-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-muted-foreground mt-1">
              Manage job postings for this organization
            </p>
          </div>
          <Button onClick={() => router.push(`/main/add-job?org=${organization.id}`)}>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-muted-foreground mb-6 max-w-md">
            Start by creating your first job posting for {organization.name}.
          </p>
          <Button onClick={() => router.push(`/main/add-job?org=${organization.id}`)}>
            <Plus className="w-4 h-4 mr-2" />
            Post Your First Job
